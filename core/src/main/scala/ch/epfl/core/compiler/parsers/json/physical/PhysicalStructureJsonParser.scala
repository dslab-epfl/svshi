...
object PhysicalStructureJsonParser {
  ...
  def constructPhysicalStructure(parsed: PhysicalStructureJson): PhysicalStructure = {
    ...
    def convertPhysicalDevice(parsed: PhysicalDeviceJson): PhysicalDevice = {
      ...
      if(addressArray.length != 3) throw new SystemStructureException(s"The address for the device $parsed is not in the right format (i.e., x.x.x)")
  ...
  def write(filePath: String, physicalStructure: PhysicalStructure): Unit = {
  ...
  def physicalStructureToJson(struct: PhysicalStructure): PhysicalStructureJson = {
    ...
    PhysicalStructureJson(struct.deviceInstances.map(device => PhysicalDeviceJson(device.name, addressToString(device.address), device.nodes.map(node => PhysicalDeviceNodeJson(node.name, node.comObjects.map(obj => PhysicalDeviceCommObjectJson(obj.name, obj.datatype.toString, obj.ioType.toString)))))))
  }
}
