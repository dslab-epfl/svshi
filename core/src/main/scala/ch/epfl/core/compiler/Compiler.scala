  ...
  def compile(library: ApplicationLibrary): ApplicationLibrary = {
    ...
    val struct = PhysicalStructure(
      PhysicalDevice(
        ...
        PhysicalDeviceNode(
          "node1",
          PhysicalDeviceCommObject("commObj1", KNXDatatype.fromString("DPT-1").get, IOType.fromString("in").get) :: PhysicalDeviceCommObject(
            "commObj10",
            KNXDatatype.fromString("DPT-12").get,
            IOType.fromString("in").get
          ) :: Nil
        ) :: Nil
      ) :: PhysicalDevice(
        "device2",
        ("1", "2", "4"),
        PhysicalDeviceNode("node2", PhysicalDeviceCommObject("commObj2", KNXDatatype.fromString("DPT-2").get, IOType.fromString("in/out").get) :: Nil) :: Nil
      ) :: Nil
    )
    ...
    Programmer.outputProgrammingFile("test", GroupAddressAssignment(struct, bindings, mapping))
    library
  }
