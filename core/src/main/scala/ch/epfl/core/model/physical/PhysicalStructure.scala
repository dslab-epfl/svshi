...
object PhysicalDeviceCommObject {
  def from(name: String, datatype: KNXDatatype, ioType: IOType, physicalAddress: (String, String, String)): PhysicalDeviceCommObject =
    PhysicalDeviceCommObject(name, datatype, ioType, MurmurHash3.listHash(List(physicalAddress, name, datatype, ioType), MurmurHash3.seqSeed))
}
