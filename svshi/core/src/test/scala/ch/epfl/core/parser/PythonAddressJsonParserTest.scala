...
import ch.epfl.core.parser.json.bindings.{AppPythonAddressesJson, BinarySensorAddressJson, DeviceAddressJson, HumiditySensorAddressJson, PythonAddressJsonParser, SwitchAddressJson, TemperatureSensorAddressJson}
...
import ch.epfl.core.model.physical.{DPT1, DPT12, DPT2, DPT5, GroupAddress, In, InOut, Out, PhysicalDevice, PhysicalDeviceCommObject, PhysicalDeviceNode, PhysicalStructure}
import ch.epfl.core.model.prototypical.{AppLibraryBindings, AppPrototypeBindings, AppPrototypicalDeviceInstance, AppPrototypicalStructure, BinarySensor, BinarySensorBinding, DeviceInstanceBinding, HumiditySensor, HumiditySensorBinding, Switch, SwitchBinding, TemperatureSensor, TemperatureSensorBinding}
...
class PythonAddressJsonParserTest extends AnyFlatSpec with Matchers {
  "assignmentToPythonAddressJson" should "return correct PythonAddressJson for correct inputs notPrivileged" in {
    ...
    val expected = AppPythonAddressesJson(permissionLevel = NotPrivileged.toString, addresses = List(
  ...
  "assignmentToPythonAddressJson" should "return correct PythonAddressJson for correct inputs privileged" in {
    val appPrototypicalStructure = AppPrototypicalStructure(permissionLevel = Privileged, deviceInstances = List(
      AppPrototypicalDeviceInstance("device1", BinarySensor),
      AppPrototypicalDeviceInstance("device2", Switch),
      AppPrototypicalDeviceInstance("device3", TemperatureSensor)
    ))
    ...
    val expected = AppPythonAddressesJson(permissionLevel = Privileged.toString, addresses = List(
      BinarySensorAddressJson("device1", "3/1/1"),
      SwitchAddressJson("device2", "2/1/1", "2/1/1"),
      TemperatureSensorAddressJson("device3", "3/2/2"),
      HumiditySensorAddressJson("device4", "3/2/1")
